...
cen_netia = cen_netia[['prefix', 'tariffdesc', 'rate EUR']]
#cen_netia.to_csv('csv\ipcall\cen_netia.csv', index = False, header=False)
#w kolumnie "Rate %" zamieniamy comma na dot
cen_netia['rate EUR'] = (cen_netia['rate EUR'].replace(',', '.', regex=True).astype(float))
#cen_netia.to_csv('csv\ipcall\cen_netia.csv', index = False, header=False)
...
cen_netia.rename(columns={'tariffdesc': 'description'}, inplace=True)
cen_netia.rename(columns={'rate EUR': 'voice_rate'}, inplace=True)


cen_netia['voice_rate'] = cen_netia['voice_rate'] * 8
cen_netia.to_csv('cen_inter_3column.csv', index=False)
...
#cen_netia['voice_rate'] = cen_netia['voice_rate'] * 8
cen_netia.to_csv('cen_test.csv', index=False)
